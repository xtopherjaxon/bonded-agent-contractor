...
const export_js_1 = require("../../utils/export.js");
/**
...
class SolanaClient {
    ...
    async exportAccount(options) {
        ...
        try {
            ...
            const { encryptedPrivateKey } = await (async () => {
                if (options.address) {
                    return index_js_1.CdpOpenApiClient.exportSolanaAccount(options.address, {
                        exportEncryptionKey: publicKey,
                    }, options.idempotencyKey);
                }
                if (options.name) {
                    return index_js_1.CdpOpenApiClient.exportSolanaAccountByName(options.name, {
                        exportEncryptionKey: publicKey,
                    }, options.idempotencyKey);
                }
                throw new errors_js_1.UserInputValidationError("Either address or name must be provided");
            })();
            const decryptedPrivateKey = (0, export_js_1.decryptWithPrivateKey)(privateKey, encryptedPrivateKey);
            return (0, export_js_1.formatSolanaPrivateKey)(decryptedPrivateKey);
        }
        catch (error) {
    ...
    async importAccount(options) {
        ...
        try {
            ...
            if (privateKeyBytes.length !== 32 && privateKeyBytes.length !== 64) {
                throw new errors_js_1.UserInputValidationError("Invalid private key length");
            }
            ...
            const openApiAccount = await index_js_1.CdpOpenApiClient.importSolanaAccount({
    ...
    async listTokenBalances(options) {
        ...
        try {
            const tokenBalances = await index_js_1.CdpOpenApiClient.listSolanaTokenBalances(options.network || "solana", options.address, {
                pageSize: options.pageSize,
                pageToken: options.pageToken,
            });
            return {
